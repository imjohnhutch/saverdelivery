...
const URL = "https://www.instacart.com/deals";

export async function scrapeInstacart(): Promise<ScrapedDeal[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`instacart direct: ${res.status}`);
    ...
    console.error("instacart fetch scraper failed:", error);
    return tryPlaywright(deals);
  }
...
async function tryPlaywright(deals: ScrapedDeal[]): Promise<ScrapedDeal[]> {
  try {
    ...
    console.warn("instacart Playwright fallback unavailable:", error instanceof Error ? error.message : error);
  }
