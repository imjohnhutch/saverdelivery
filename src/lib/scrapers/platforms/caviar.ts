...
const URL = "https://www.trycaviar.com/offers";

export async function scrapeCaviar(): Promise<ScrapedDeal[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`caviar direct: ${res.status}`);
    ...
    console.error("caviar fetch scraper failed:", error);
    return tryPlaywright(deals);
  }
...
async function tryPlaywright(deals: ScrapedDeal[]): Promise<ScrapedDeal[]> {
  try {
    ...
    console.warn("caviar Playwright fallback unavailable:", error instanceof Error ? error.message : error);
  }
