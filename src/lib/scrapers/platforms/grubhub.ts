...
const URL = "https://www.grubhub.com/deals";

export async function scrapeGrubhub(): Promise<ScrapedDeal[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`grubhub direct: ${res.status}`);
    ...
    console.error("grubhub fetch scraper failed:", error);
    return tryPlaywright(deals);
  }
...
async function tryPlaywright(deals: ScrapedDeal[]): Promise<ScrapedDeal[]> {
  try {
    ...
    console.warn("grubhub Playwright fallback unavailable:", error instanceof Error ? error.message : error);
  }
