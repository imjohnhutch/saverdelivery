...
const URL = "https://www.ubereats.com/offers";

export async function scrapeUberEats(): Promise<ScrapedDeal[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`ubereats direct: ${res.status}`);
    ...
    console.error("ubereats fetch scraper failed:", error);
    return tryPlaywright(deals);
  }
...
function parseHtml($: cheerio.CheerioAPI, deals: ScrapedDeal[]): boolean {
  ...
  $('[class*="promo"], [class*="offer"], [class*="deal"], [class*="coupon"], [data-testid*="promo"], [data-testid*="offer"]').each(
    (_, el) => {
      try {
...
async function tryPlaywright(deals: ScrapedDeal[]): Promise<ScrapedDeal[]> {
  try {
    ...
    console.warn("ubereats Playwright fallback unavailable:", error instanceof Error ? error.message : error);
  }
