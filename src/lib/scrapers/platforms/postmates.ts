...
const URL = "https://postmates.com/offers";

export async function scrapePostmates(): Promise<ScrapedDeal[]> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`postmates direct: ${res.status}`);
    ...
    console.error("postmates fetch scraper failed:", error);
    return tryPlaywright(deals);
  }
...
async function tryPlaywright(deals: ScrapedDeal[]): Promise<ScrapedDeal[]> {
  try {
    ...
    console.warn("postmates Playwright fallback unavailable:", error instanceof Error ? error.message : error);
  }
